...
const fetchData = async (setJsonData) => {
    try {
        ...
        const response =  await ApiRequest('http://165.227.97.46:8000/api/load', getOptions)
...
const saveData = async (hotRef) => {
    try {
        ...
        const response =  await ApiRequest('http://165.227.97.46:8000/api/save', postOptions)
    }catch (err){
    }
}
